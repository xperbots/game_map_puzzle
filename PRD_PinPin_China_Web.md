...
### 2.2 功能模块
本游戏包含以下最小页面集合：
1. **首页**：开始游戏 (StartScene)
2. **第一关 (Level 1)**：省份拼拼看-简单模式 (Easy)，磁力拼图，含辅助提示
3. **第二关 (Level 2)**：省份拼拼看-挑战模式 (Hard)，无省界提示、无悬停提示、需精确释放
4. **结算页**：恭喜/统计页 (EndScene)，显示总耗时并支持重玩
5. **HUD**：全局计时器

### 2.3 页面详情
| 页面 | 模块 | 功能描述 |
|------|------|----------|
| 首页 | 开始按钮 | 进入第一关，开启全局计时 |
| **第一关 (Easy)** | 拼图槽 | 显示**省份轮廓**与吸附反馈 |
| 第一关 | 拼图块 | 可拖拽、**悬停提示**、自动吸附、播放音效 |
| 第一关 | 标签 | **始终显示**省份简称 |
| 第一关 | HUD | 顶部显示已用时间 |
| **第二关 (Hard)** | 拼图槽 | 仅显示**中国外轮廓**，无省界线 |
| 第二关 | 拼图块 | 可拖拽、**无悬停提示**、需**精确释放** |
| 第二关 | 标签 | **仅直辖市显示**，其他省份拼对后显示 |
| 第二关 | HUD | 继续显示累计时间 |
| **结算页** | 通关信息 | 显示“恭喜通关”及总耗时 |
| 结算页 | 重玩按钮 | 返回首页，重置计时 |

## 3. 核心流程
游客打开首页 → 点击开始（计时开始）→ 第一关（简单模式）→ 完成 → 第二关（挑战模式）→ 完成 → 结算页（展示时间/重玩）。

```mermaid
graph TD
  A["首页 (StartScene)"] --> B["第一关: 省份拼拼看-简单 (GameScene)"]
  B --> C["第二关: 省份拼拼看-挑战 (Level2HardScene)"]
  C --> D["结算页 (EndScene)"]
  D --> A
```
