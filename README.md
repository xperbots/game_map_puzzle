...
### Level 1: Magnetic Map (磁力拼图)
*   **目标**: 还原中国地图。
*   **玩法**: 34 个省份拼图散落在屏幕两侧，玩家需要将其拖拽到地图中央的正确位置。拼图接近正确位置时会自动吸附 (Magnetic Snap)。
*   **特色**: 
    *   **柔和配色 (Pastel)**: 采用莫兰迪色系，视觉舒适现代。
    *   **五星红旗 (Flag Effect)**: 拼图归位后瞬间平整化，且变为**国旗红**，最终汇聚成一面完整的五星红旗。
    *   **智能标注**: 自动显示省份简称（如“新疆”、“上海”），文字始终浮于最上层，清晰易读。
    *   **伪 3D 积木**: 拼图块拥有立体厚度，吸附时平滑过渡为平面国旗。

### Level 2: Hard Mode (挑战模式)
*   **目标**: 依靠形状记忆还原地图。
*   **挑战升级**:
    *   **无边界 (No Borders)**: 地图底板仅保留中国外轮廓，不再显示省份界线。
    *   **精准吸附 (Precision)**: 取消吸附提示，拼图必须非常精确地放置（距离误差 < 15%）才能吸附。
    *   **无提示 (No Hints)**: 移除且拖拽时的高亮提示。
    *   **隐形标签**: 默认隐藏省份名称，仅在拼对后显示（直辖市与港澳除外）。

### Level 3: Capital Rain (省会雨)
*   **目标**: 匹配省份形状与其对应的省会名称。
*   **玩法**: 屏幕上方不断掉落“省份形状”和“省会名称”方块。玩家需要点击选这两个对应的方块进行消除。
*   **挑战**: 
    *   **Miss 惩罚**: 掉落到底部会扣除生命值 (初始 5 命)。
    *   **错误惩罚**: 匹配错误会震动屏幕并播放错误音效。
    *   **难度递增**: 随着时间推移，掉落速度会逐渐加快。
